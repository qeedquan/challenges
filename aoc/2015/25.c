...
void
readints(const char *name, long *row, long *col)
{
	FILE *fp;
	...
	*row = *col = 0;
	fscanf(fp, "To continue, please consult the code grid in the manual.  Enter the code at row %ld, column %ld.", row, col);
	fclose(fp);
}

long
solve(long row, long col)
{
	long index;
	long code;
	long i;

	index = triangular(row + col - 2) + col;
	code = 20151125;
	for (i = 1; i < index; i++)
