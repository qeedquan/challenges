...
func part1(value uint64) uint64 {
	return next(value, 0)
}

func part2(value uint64) uint64 {
	prev := uint64(0)
	hash := uint64(0)
...
func next(seed, hash uint64) uint64 {
	a := seed
	b := hash | 0x10000
	for range 3 {
		a = (a + (b & 0xff)) & 0xffffff
...
func load(name string) (uint64, error) {
	...
	for _, line := range lines {
		_, err := fmt.Sscanf(line, "seti %d", &value)
		if err == nil {
			if count++; count == 4 {
				break
			}
