...
def readkey(name):
    file = open(name)
    card_pub = int(file.readline())
    door_pub = int(file.readline())
    file.close()
    return card_pub, door_pub

def solve(card_pub, door_pub):
    modulus = 20201227
    return pow(door_pub, discrete_log(modulus, card_pub, 7), modulus)

def main():
    card_pub, door_pub = readkey("25.txt")
    print(solve(card_pub, door_pub))
