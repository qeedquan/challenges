...
void
load(const char *name, Grid grid, int *width, int *height)
{
	FILE *fp;
	int x, y;
	int c;
	...
	*width = *height = 0;
	...
	for (;;) {
		...
		if (x >= SIZE || y >= SIZE)
			fatal("Failed to load file: out of range: (%d, %d)", x, y);
...
int
right(Grid grid, Grid grid2, int width, int, int x, int y)
{
	int nx;
...
int
down(Grid grid, Grid grid2, int, int height, int x, int y)
{
	int ny;
...
int
traverse(Grid grid, int width, int height, int (*move)(Grid, Grid, int, int, int, int))
{
	Grid grid2;
	int moved;
	int x, y;
	...
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++)
			moved += move(grid, grid2, width, height, x, y);
	}
...
int
solve(Grid grid, int width, int height)
{
	int step;
	int moved;

	for (step = 1; step < INT_MAX; step++) {
		moved = traverse(grid, width, height, right);
		moved += traverse(grid, width, height, down);
		if (moved == 0)
			return step;
	}
...
main()
{
	...
	load("25.txt", grid, &width, &height);
	printf("%d\n", solve(grid, width, height));
