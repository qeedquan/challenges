...
def rating(data, less):
    index = 0
    while data.shape[0] > 1:
        column = data[:, index]
        more_ones = occurrence(column, less)
        data = data[column == more_ones]
        index += 1
    return binary_array_to_int(data[0])

def part1(data):
    more_ones = occurrence(data, False)
    gamma = binary_array_to_int(more_ones.astype(int))
    epsilon = binary_array_to_int((~more_ones).astype(int))
    return gamma * epsilon    

def part2(data):
    oxygen = rating(data, False)
    carbon = rating(data, True)
