...
func main() {
	splitters, err := load("7.txt")
	if err != nil {
		log.Fatal(err)
	}

	result1, result2 := solve(splitters)
	fmt.Println(result1)
	fmt.Println(result2)
}

func solve(splitters []int) (int64, int64) {
	entering := make([]int64, len(splitters))
	entering[0] = 1
