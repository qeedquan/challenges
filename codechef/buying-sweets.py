...
Ann is working as a sweet seller at a shop in Strangeland.
Every kind of sweets in this shop has its own cost, and sweets of the same kind have the same cost.
...
def main():
    assert(sweets([10, 4, 8, 5], 7) == -1)
    assert(sweets([12], 10) == 1)
    assert(sweets([20, 50], 10) == 7)
