...
Each test case contains exactly one line. The line consists of four integers x, y, a, b (0≤x<y≤10^9, 1≤a,b≤10^9) —
...
In the second case, each rabbit will be at position 3 and 7 respectively at the 1-st second.
But in the 2-nd second they will be at 6 and 4 respectively, and we can see that they will never be at the same position since the distance between the two rabbits will only increase afterward.

"""

def meeting_time(x, y, a, b):
    z = y - x
    c = a + b
    if c == 0 or z%c != 0:
        return -1
...
def main():
    assert(meeting_time(0, 10, 2, 3) == 2)
    assert(meeting_time(0, 10, 3, 3) == -1)
    assert(meeting_time(900000000, 1000000000, 1, 9999999) == 10)
    assert(meeting_time(1, 2, 1, 1) == -1)
    assert(meeting_time(1, 3, 1, 1) == 1)
