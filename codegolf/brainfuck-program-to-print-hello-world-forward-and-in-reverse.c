...
I am looking to create a brainfuck program that can print the text "Hello, World" when executed--but not just that (it's too easy), it should also print "dlroW ,olleH" when the program is executed backwards. i.e

This code probably prints hello world.

>++++++++[<+++++++++>-]<.>++++[<+++++++>-]<+.+++++++..+++.>>++++++[<+++++++>-]<+
+.------------.>++++++[<+++++++++>-]<+.<.+++.------.--------.>>>++++[<++++++++>-
]<+.
but now I need a version such that when executed from the last character (. in this case) prints the same text in reverse.

Specifically, I need a hello world program such that when I run it through the rev command, it prints "dlroW ,olleH".

Shortest code wins! Better if you find a way to use loops.

Current implementation written in C.

Negative pointers not allowed.
Unlimited expanding memory.
Understands only the eight brainfuck symbols and ignores everything else.
/ *
...
Explanation: The characters dW ,orleH or Helro, Wd respectively are loaded into consecutive cells by the code at the start/reversed code at the end and
then the code in the middle outputs the desired cells, repeating the l and o as they're the same distance away in either direction.
