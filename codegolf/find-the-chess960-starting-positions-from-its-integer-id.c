...
Chess960, a.k.a. Fisher random chess, is a variation of the game of chess.
...
init()
{
	...
	for (lineno = 0; lineno < nelem(LINES); lineno++) {
		snprintf(line, sizeof(line), "%s", LINES[lineno]);
		ptr = line;
		for (count = 0; count < 10; count++) {
			index = atoi(strtok(ptr, " "));
			fen = strtok(NULL, " ");
			snprintf(C960[index], sizeof(C960[index]), "%s", fen);
			ptr = NULL;
		}
	}
}
...
checksum()
{
	...
	result = 0;
	for (index = 0; index < nelem(C960); index++)
		result += (960 + index) * value(C960[index]);
	return result;
}
...
{
	init();
	assert(checksum() == 38292381401040);
	return 0;
}
