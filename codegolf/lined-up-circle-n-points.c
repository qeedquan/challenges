...
initsdl(void)
{
	...
	if (!SDL_CreateWindowAndRenderer("Circle Complete Graph", 800, 800, SDL_WINDOW_RESIZABLE, &window, &renderer))
...
rootsofunity(Region *r, int i)
{
	...
	c = cexpf(2 * SDL_PI_F * I * i / r->npoints);
	return (SDL_FPoint){
...
drawregion(Region *r)
{
	...
	for (i = 0; i < r->npoints; i++) {
		...
		for (j = i + 1; j < r->npoints; j++) {
			q = rootsofunity(r, j);
			SDL_RenderLine(renderer, p.x, p.y, q.x, q.y);
		}
	}
}
