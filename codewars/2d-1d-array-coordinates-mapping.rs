...
fn main() {
    assert_eq!(to1d(0, 0, (3, 3)), 0);
    assert_eq!(to1d(1, 1, (3, 3)), 4);
    assert_eq!(to1d(2, 2, (3, 3)), 8);

    assert_eq!(to2d(5, (3, 3)), (2, 1));
    assert_eq!(to2d(3, (3, 3)), (0, 1));
}

fn to1d(x: isize, y: isize, size: (isize, isize)) -> isize {
    (y * size.0) + x
}

fn to2d(n: isize, size: (isize, isize)) -> (isize, isize) {
    (n % size.0, n / size.0)
}
