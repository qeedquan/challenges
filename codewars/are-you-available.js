...
function checkAvailability(schedule, time) {
	for (let index = 0; index < schedule.length; index++) {
		if (schedule[index][0] <= time && time <= schedule[index][1])
			return schedule[index][1];
		if (schedule[index][0] > time)
			break;
	}
	return true;
}

console.log(checkAvailability([["09:30", "10:15"], ["12:20", "15:50"]], "11:00"));
