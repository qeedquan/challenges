...
void
uncensor(const char *infected, const char *discovered, char *correction)
{
...
void
test(const char *infected, const char *discovered, const char *expected)
{
	char correction[32];

	uncensor(infected, discovered, correction);
	puts(correction);
	assert(!strcmp(correction, expected));
}

int
main()
{
	test("*h*s *s v*ry *tr*ng*", "Tiiesae", "This is very strange");
	test("A**Z*N*", "MAIG", "AMAZING");
	test("xyz", "", "xyz");
