...
func main() {
	fmt.Println(change("John", []float64{1.1, 2.2, 3.3}, 10))
	fmt.Println(change("John", []float64{11}, 10))
}

func change(name string, goods []float64, payment float64) float64 {
	total := 0.0
	...
	if strings.ToLower(name) == "john" {
		result = 2*result - math.Floor(result)
	}
	return result
}
