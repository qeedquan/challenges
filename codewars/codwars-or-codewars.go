...
func main() {
	assert(findcodwars("codwars.com") == true)
	assert(findcodwars("https://subdomain.codwars.com/a/sub/directory?a=querystring") == true)
	assert(findcodwars("codewars.com") == false)
	assert(findcodwars("https://subdomain.codwars.codewars.com/a/sub/directory/codwars.com?a=querystring") == false)
}
...
func findcodwars(addr string) bool {
	url, err := url.Parse(addr)
	if err != nil {
		return false
	}

	for _, path := range []string{url.Host, url.Path} {
		index0 := strings.Index(path, "codwars.com")
		index1 := strings.Index(path, "codewars.com")
		switch {
		case index0 < 0 && index1 < 0:
			continue
		case index0 >= 0 && index1 < 0:
			return true
		case index0 < 0 && index1 >= 0:
