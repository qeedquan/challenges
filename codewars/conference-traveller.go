...
func main() {
	assert(pick(
		[]string{"Mexico City", "Johannesburg", "Stockholm", "Osaka", "Saint Petersburg", "London"},
		[]string{"Stockholm", "Paris", "Melbourne"},
	) == "Paris")
}
...
func pick(visited, offered []string) string {
	seen := make(map[string]bool)
	for _, city := range visited {
		seen[city] = true
	}
	for _, city := range offered {
		if !seen[city] {
			return city
		}
	}
