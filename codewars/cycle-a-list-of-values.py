...
def cycle(direction, array, current):
    try:
        move = direction % len(array)
...
def main():
    assert(cycle(1, [1, 2, 3], 1) == 2)
    assert(cycle(-1, [1, 2, 3], 1) == 3)
    assert(cycle(1, [1, 2, 3], 0) == None)
    assert(cycle(1, [1, 2, 2, 3], 2) == 2)
