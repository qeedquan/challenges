...
fn main() {
    assert_eq!(whatnote("e", 0), "E");
    assert_eq!(whatnote("D", 5), "G");
    assert_eq!(whatnote("E", 18), "A#");
}

fn whatnote(string: &'static str, fret: usize) -> &'static str {
    let notes = vec![
        "F", "F#", "G", "G#", "A", "A#", "B", "C", "C#", "D", "D#", "E",
    ];
    let string = string.to_ascii_uppercase();
    for index in 0..notes.len() {
        if string == notes[index] {
            return notes[(index + fret) % notes.len()];
        }
    }
    return "invalid";
}
