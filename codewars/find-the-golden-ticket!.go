...
func main() {
	assert(finder([][]bool{{false}, {false}, {false}, {true}, {false}}) == 3)
	assert(finder([][]bool{{true}, {false}, {false}, {false}}) == 0)
	assert(finder([][]bool{{false}, {false}, {false}, {false}, {false}}) == "There is no golden ticket!")
}
...
func finder(box [][]bool) any {
	for i := range box {
		if len(box[i]) > 0 && box[i][0] {
			return i
		}
	}
