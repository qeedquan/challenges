...
Number of tests: 100
Arrays of length between 30 and 100q
...
def process(array):
    number0 = ''.join(sorted(array))
    counts = Counter(array)
    product = 1
    for value in counts.values():
        product *= factorial(value)
    
    number1 = number0.lstrip('0')
    if len(number1) == 0:
        number1 = '0'

    return [factorial(len(array)) // product, int(number1), int(number0[::-1])]

def main():
    assert(process(['1', '2', '2', '3', '2', '3']) == [60, 122233, 332221])
    assert(process(['1', '2', '3', '0', '5', '1', '1', '3']) == [3360, 1112335, 53321110])
