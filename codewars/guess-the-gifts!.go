...
func main() {
	...
	fmt.Println(guessgifts(wishlist, presents))
}

func guessgifts(wishlist, presents []Item) []string {
	table := make(map[[3]string][]string)
	for _, item := range wishlist {
		key := [3]string{item.size, item.clatters, item.weight}
		table[key] = append(table[key], item.name)
	}
