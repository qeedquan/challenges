...
If you think of a number, between 1 and 31, I can tell what number you are thinking, just by asking you in which tuple you can find that number. You don't believe me? What if I tell you the trick?
...
So for example, let's say I am thinking about the number 5. It can be found in the first and third tuple, which have respectively the numbers 1 and 4 as first element. The sum of 1 and 4 is 5!
...
func main() {
	assert(guess([5]int{1, 0, 1, 0, 0}) == 5)
	assert(guess([5]int{1, 0, 0, 0, 0}) == 1)
	assert(guess([5]int{1, 1, 0, 1, 0}) == 11)

	assert(answer(1) == [5]int{1, 0, 0, 0, 0})
	assert(answer(12) == [5]int{0, 0, 1, 1, 0})
}
...
func answer(n int) [5]int {
	var r [5]int
	for i, s := range seq {
		if slices.Index(s, n) >= 0 {
			r[i] = 1
		}
	}
	return r
