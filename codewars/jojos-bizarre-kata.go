...
func main() {
	assert(rejojo.MatchString("Jonathan Joestar") == true)
	assert(rejojo.MatchString("Dio Brando") == false)
	assert(rejojo.MatchString("George Joestar II") == false)
	assert(rejojo.MatchString("Giorno Giovanna") == true)
	assert(rejojo.MatchString("Josuke Joestar") == true)
	assert(rejojo.MatchString("Kars") == false)
	assert(rejojo.MatchString("Caesar Zeppeli") == false)
}
...
var rejojo = regexp.MustCompile(`(?i)^(Jo\w+ Jo\w+|gio\w+ gio\w+|Jo\w+ \w+jo)$`)
