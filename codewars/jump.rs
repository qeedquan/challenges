...
fn main() {
    assert_eq!(jumpable(&vec![6, 1, 1]), true);
    assert_eq!(jumpable(&vec![6]), false);
    assert_eq!(jumpable(&vec![1, 1, 3]), false);
    assert_eq!(jumpable(&vec![5]), false);
    assert_eq!(jumpable(&vec![2, 5]), true);
    assert_eq!(jumpable(&vec![3, 0, 2, 3]), true);
    assert_eq!(jumpable(&vec![4, 1, 2, 0, 1]), false);
    assert_eq!(jumpable(&vec![5, 0, 0, 0]), true);
    assert_eq!(jumpable(&vec![1, 1]), false);
}

fn jumpable(a: &Vec<usize>) -> bool {
    recurse(a, 0)
}

fn recurse(a: &Vec<usize>, n: usize) -> bool {
    if n + 1 >= a.len() || a[n] == 0 {
        return false;
    }
    for i in n + 1..n + a[n] + 1 {
        if i >= a.len() || recurse(a, i) {
            return true;
        }
    }
    return false;
