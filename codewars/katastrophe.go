...
func main() {
	assert(strongenough([][]int{{2, 3, 1}, {3, 1, 1}, {1, 1, 2}}, 2) == "Safe!")
	assert(strongenough([][]int{{5, 8, 7}, {3, 3, 1}, {4, 1, 2}}, 2) == "Safe!")
	assert(strongenough([][]int{{5, 8, 7}, {3, 3, 1}, {4, 1, 2}}, 3) == "Needs Reinforcement!")
}
...
func strongenough(earthquake [][]int, age int) string {
	if len(earthquake) == 0 {
		return "Safe!"
	}

	magnitude := 1
	for i := range earthquake {
		sum := 0
		for j := range earthquake[i] {
			sum += earthquake[i][j]
		}
		magnitude *= sum
	}

	strength := float64(1000)
	for ; age > 0; age-- {
		strength -= strength / 100
	}

	if float64(magnitude) > strength {
