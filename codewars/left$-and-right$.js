...
function left$(str, i=1) {
	if (typeof i === 'string')
		return str.slice(0, str.indexOf(i));
	if (i == 0)
		return "";
...
function right$(str, i=1) {
	if (typeof i === 'string')
		return str.slice(str.lastIndexOf(i) + i.length);
	if (i == 0)
		return "";
	return str.slice(-i);
}
...
function main() {
	var str = 'Hello (not so) cruel World!'
	assert(left$(str, 5) == "Hello");
	assert(left$(str, -22) == "Hello");
	assert(left$(str, 1) == "H");
	assert(left$(str) == "H");
	assert(left$(str, 0) == "");
	assert(left$(str, 99) == "Hello (not so) cruel World!");

	assert(right$(str, 6) == "World!");
	assert(right$(str) == "!");

	assert(left$(str, "o") == "Hell");
	assert(right$(str, "o") == "rld!");
	assert(left$(str, " ") == "Hello");
}
