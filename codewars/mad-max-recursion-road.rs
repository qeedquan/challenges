...
fn main() {
    assert_eq!(madmax(&vec![1, 42, 112, 32, 21]), 112);
}

fn madmax(array: &Vec<isize>) -> isize {
    return recurse(array, 0);
}

fn recurse(array: &Vec<isize>, index: usize) -> isize {
    if index >= array.len() {
        return isize::MIN;
    }
    max(array[index], recurse(array, index + 1))
}
