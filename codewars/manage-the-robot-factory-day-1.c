...
int
calculate_scraps(int *scraps, size_t nscraps, int robots)
{
	double mass;
	size_t i;
	...
	mass = robots * 50;
	for (i = 0; i < nscraps; i++)
		mass *= 100.0 / (100 - scraps[i]);
	return ceil(mass);
}

void
test(int *scraps, size_t nscraps, int robots, int expected)
{
	int mass;

	mass = calculate_scraps(scraps, nscraps, robots);
	printf("%d\n", mass);
...
main()
{
	...
	test(scraps_1, nelem(scraps_1), 90, 5000);
	test(scraps_2, nelem(scraps_2), 55, 3820);
