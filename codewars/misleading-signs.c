...
bool
gaslighting(const char *shirt, const char *word, const char *letters)
{
	bool known[256];

	memset(known, 0, sizeof(known));
	for (; *letters; letters++)
		known[*letters & 0xff] = true;

	for (; *shirt && *word; shirt++, word++) {
		if (*shirt != *word && (known[*shirt & 0xff] || known[*word & 0xff]))
			return true;
	}
	return false;
}
...
main()
{
	assert(gaslighting("snack", "snake", "c") == true);
	assert(gaslighting("snack", "snack", "snack") == false);
	assert(gaslighting("snake", "snack", "c") == true);
	assert(gaslighting("smile", "smirk", "smi") == false);
