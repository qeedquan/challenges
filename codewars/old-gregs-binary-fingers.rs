...
First he sticks his thumb out which makes 1, then just his index finger which makes 10 (decimal 2),
then Index and thumb which makes 11 (decimal 3), then just his middle finger which makes 100 (decimal 4)
and so on up to all five fingers out which makes 11111 (decimal 31).
...
You need to create a function: binaryFingers into which will be passed a string of 1s and 0s,
it should return an array showing which of Old Greg's digits which are up, left to right, as Old Greg sees them:
...
fn main() {
    assert_eq!(binary_fingers("101"), vec!["Middle", "Thumb"]);
    assert_eq!(
        binary_fingers("11111"),
        vec!["Pinkie", "Ring", "Middle", "Index", "Thumb"]
    );
}

fn binary_fingers(string: &'static str) -> Vec<&'static str> {
    let table = ["Pinkie", "Ring", "Middle", "Index", "Thumb"];
    let mut result = vec![];
    let mut index = 0;
    for symbol in string.chars() {
        ...
        if symbol == '1' {
            result.push(table[table.len() - index - 1]);
        }
        index += 1;
    }
    result.reverse();
