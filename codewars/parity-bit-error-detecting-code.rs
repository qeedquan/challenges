...
fn main() {
    assert_eq!(parity("01011001"), "0101100");
    assert_eq!(parity("01101110 01100000"), "error 0110000");
    assert_eq!(
        parity("10100011 00111001 11001100"),
        "1010001 0011100 1100110"
    );
    assert_eq!(
        parity("01011001 01101110 01100000 01010110 10001111 01100011"),
        "0101100 error 0110000 0101011 error 0110001",
    );
}

fn parity(data: &'static str) -> String {
    let mut result = String::new();
    'next: for bits in data.split(' ') {
        let (mut zeros, mut ones, mut checkbit) = (0, 0, 0);
        for bit in bits.chars() {
            if bit == '0' {
                (zeros, checkbit) = (zeros + 1, 0);
            } else if bit == '1' {
                (ones, checkbit) = (ones + 1, 1);
            } else {
                (zeros, ones) = (0, 0);
                break;
            }
        ...
        if ((ones ^ checkbit) & 1) != 0 {
            result = format!("{result}error ");
        } else {
            result = format!("{result}{} ", &bits[0..7]);
        }
    }
    format!("{}", result.trim())
}
