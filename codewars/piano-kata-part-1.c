...
const char *
colorkey(int keypresses)
{
	keypresses = mod(keypresses - 1, 88);
	switch (mod(keypresses, 12)) {
	case 1:
	case 4:
	case 6:
	case 9:
...
void
test(int keypresses, const char *expected)
{
	const char *color;

	color = colorkey(keypresses);
	puts(color);
	assert(!strcmp(color, expected));
}
