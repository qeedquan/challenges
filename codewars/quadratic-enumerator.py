...
def quadratic(a, c, b, start=0, step=1):
    x, n = start, step
    while True:
        yield (x, a*x**2 + b*x + c)
        x += n

def iter(generator, limit=10):
    index = 0
...
def main():
    iter(quadratic(1, 0, 0))
    iter(quadratic(1, 0, 0, start=2))
    iter(quadratic(1, 0, 0, step=2))
    iter(quadratic(1, 0, 0, step=-1))
    iter(quadratic(1, 0, 0, step=0.5))
