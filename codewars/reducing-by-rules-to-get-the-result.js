...
function reduceByRules(numbers, rules) {
	return numbers.reduce((current, number, index) => rules[(index - 1) % rules.length](current, number));
}

let numbers = [2.0, 2.0, 3.0, 4.0];
let rules = [(a,b) => a + b, (a,b) => a - b];
console.log(reduceByRules(numbers, rules));
