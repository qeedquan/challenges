...
func relatively_prime(_ n: Int, _ a: [Int]) -> [Int] {
    var r = [Int]()
    for x in a {
        if gcd(n, x) == 1 {
            r.append(x)
...
assert(relatively_prime(8, [1, 2, 3, 4, 5, 6, 7]) == [1, 3, 5, 7])
assert(relatively_prime(15, [72, 27, 32, 61, 77, 11, 40]) == [32, 61, 77, 11])
assert(relatively_prime(210, [15, 100, 2_222_222, 6, 4, 12369, 99]) == [])
