...
fn main() {
    assert_eq!(wheat_from_chaff(&mut [7, -8, 1, -2]), &[-2, -8, 1, 7]);
    assert_eq!(
        wheat_from_chaff(&mut [-31, -5, 11, -42, -22, -46, -4, -28]),
        &[-31, -5, -28, -42, -22, -46, -4, 11]
    );
    assert_eq!(
        wheat_from_chaff(&mut [-25, -48, -29, -25, 1, 49, -32, -19, -46, 1]),
        &[-25, -48, -29, -25, -46, -19, -32, 49, 1, 1]
    );
}

fn wheat_from_chaff(values: &mut [isize]) -> &[isize] {
    if values.len() == 0 {
        return values;
    }

    let mut left = values.len() - 1;
    let mut right = 0;
    while right < left {
