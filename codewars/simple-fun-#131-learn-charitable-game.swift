...
func charitable(_ array: [Int]) -> Bool {
    let length = array.count
    if length == 0 {
        return false
    }
    let total = array.reduce(0, +)
    return total > 0 && (total % length == 0)
}

assert(charitable([0, 56, 100]) == true)
assert(charitable([0, 0, 0]) == false)
assert(charitable([11]) == true)
assert(charitable([100, 100, 100, 90, 1, 0, 0]) == false)
