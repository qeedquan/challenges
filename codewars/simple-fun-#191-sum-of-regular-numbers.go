...
func main() {
	assert(sorn([]int{54, 70, 86, 1, -2, -5, 0, 5, 78, 145, 212, 15}) == 639)
	assert(sorn([]int{1, 2, 3, 4, 5}) == 15)
	assert(sorn([]int{59, 58, 57, 55, 53, 51}) == 390)
	assert(sorn([]int{7, 2, -3, 3, 9, 15}) == 30)
	assert(sorn([]int{-17, -9, 1, 9, 17, 4, -9}) == 39)
	assert(sorn([]int{7, 2, 3, 2, -2, 400, 802}) == 1200)
	assert(sorn([]int{-1, 7000, 1, -6998, -13997}) == -13994)
}
...
func sorn(a []int) int {
	...
	for i := 0; i < len(a)-2; {
		...
		s := a[i]
		c := 1
		for j := i + 1; j < len(a) && a[j]-a[j-1] == d; j++ {
			s += a[j]
			c += 1
		}
