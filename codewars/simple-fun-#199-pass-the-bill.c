...
int
billpass(int total, int conservatives, int reformist)
{
	int needed;
	int independent;

	needed = (total / 2) + 1;
	independent = total - conservatives - reformist;
	if (needed <= conservatives)
		return 0;
	if (needed - conservatives > independent)
		return -1;
	return needed - conservatives;
}

int
main()
{
	assert(billpass(8, 3, 3) == 2);
	assert(billpass(13, 4, 7) == -1);
	assert(billpass(7, 4, 3) == 0);
	assert(billpass(11, 4, 1) == 2);
	assert(billpass(11, 5, 1) == 1);
	assert(billpass(11, 6, 1) == 0);
	assert(billpass(11, 4, 4) == 2);
	assert(billpass(11, 5, 4) == 1);
	assert(billpass(11, 5, 5) == 1);
	assert(billpass(11, 4, 6) == -1);
	assert(billpass(11, 4, 5) == 2);
	assert(billpass(15, 9, 3) == 0);
	return 0;
}
