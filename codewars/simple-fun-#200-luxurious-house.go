...
func main() {
	test([]int{1, 2, 3, 1, 2}, []int{3, 2, 0, 2, 0})
	test([]int{3, 2, 1, 4}, []int{2, 3, 4, 0})
	test([]int{1, 2, 3}, []int{3, 2, 0})
	test([]int{3, 2, 1}, []int{0, 0, 0})
	test([]int{1, 1, 1}, []int{1, 1, 0})
}
...
func test(houses, expected []int) {
	result := luxhouse(houses)
	fmt.Println(result)
	assert(reflect.DeepEqual(result, expected))
}

func luxhouse(houses []int) []int {
