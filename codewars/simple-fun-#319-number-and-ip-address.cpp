...
string ip2addr(const string &ip)
{
	unsigned addr[4];
	if (sscanf(ip.c_str(), "%u.%u.%u.%u", &addr[3], &addr[2], &addr[1], &addr[0]) != 4)
		return "";
	return format("{0}", addr[0] | (addr[1] << 8) | (addr[2] << 16) | (addr[3] << 24));
}

string addr2ip(const string &addr)
{
	auto ip = strtol(addr.c_str(), NULL, 10);
	return format("{0}.{1}.{2}.{3}", (ip >> 24) & 0xff, (ip >> 16) & 0xff, (ip >> 8) & 0xff, ip & 0xff);
}

int main()
{
	assert(ip2addr("10.0.3.193") == "167773121");
	assert(addr2ip("167969729") == "10.3.3.193");
	return 0;
}
