...
Notes
tradeType will either be 'LONG' or 'SHORT'

You will need to check that the stoploss given is valid.
...
riskPercentage is the risk expressed as a percentage. e.g 2.5 means 2.5%
...
func main() {
	test("SHORT", 5, 5.50, 5000, 5, 500)
	test("LONG", 3, 2.95, 10000, 2, 4000)
	test("SHORT", 32.50, 33, 12500, 4.5, 1125)
	test("LONG", 3, 3.00, 10000, 2, math.NaN())
	test("SHORT", 4.54, 4.53, 6000, 3.75, math.NaN())
}
...
func test(tradetype string, entryprice, stoploss, capital, riskpercent, expected float64) {
	result := positionsize(tradetype, entryprice, stoploss, capital, riskpercent)
	fmt.Println(result)
...
func positionsize(tradetype string, entryprice, stoploss, capital, riskpercent float64) float64 {
	switch {
	case tradetype == "LONG" && entryprice <= stoploss:
		fallthrough
	case tradetype == "SHORT" && entryprice >= stoploss:
		return math.NaN()
	}

	difference := math.Abs(entryprice - stoploss)
	percent := (capital / 100) * riskpercent
	return math.Round(percent / difference)
}
