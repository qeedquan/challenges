...
int
tram(int stops, int descending[], int onboarding[])
{
	int stop;
	int max;
	int cur;
	...
	for (stop = 0; stop < stops; stop++) {
		cur = cur - descending[stop] + onboarding[stop];
		if (cur > max)
			max = cur;
	}
	return max;
...
main()
{
	int descending[] = {0, 2, 4, 4};
	int onboarding[] = {3, 5, 2, 0};

	assert(tram(nelem(descending), descending, onboarding) == 6);
	return 0;
}
