...
func main() {
	fmt.Println(transpose([]string{"G"}, 5))
	fmt.Println(transpose([]string{"Db"}, -4))
	fmt.Println(transpose([]string{"E", "F"}, 1))
}

func transpose(songs []string, interval int) []string {
	notes0 := []string{"A", "Bb", "B", "C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab"}
	notes1 := []string{"A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"}
	result := []string{}
	for _, song := range songs {
		index := slices.Index(notes0, song)
		if index < 0 {
			index = slices.Index(notes1, song)
		}
		index += interval
		if index < 0 {
			index += 12
		...
		result = append(result, notes1[index])
	}
	return result
}
