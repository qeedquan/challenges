...
const char *
calculate(double distance, double busdrive, double buswalk)
{
	double bus;
	double walk;

	bus = (buswalk / 5) + (busdrive / 8);
	walk = distance / 5;
	if (walk > 2 || (bus < walk && (walk * 60) > 10))
		return "Bus";
	return "Walk";
}

void
test(double distance, double busdrive, double buswalk, const char *expected)
{
	const char *result;

	result = calculate(distance, busdrive, buswalk);
	puts(result);
