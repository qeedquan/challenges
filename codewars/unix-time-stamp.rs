...
Background
Unix Time ( https://en.wikipedia.org/wiki/Unix_time ) Stamp is a date and time representation widely used in computing.
...
fn main() {
    assert_eq!(unixtime([1970, 1, 1, 0, 0, 0]), 0);
    assert_eq!(unixtime([2001, 9, 9, 1, 46, 40]), 1000000000);
    assert_eq!(unixtime([2345, 6, 7, 8, 9, 10]), 11847456550);
    assert_eq!(unixtime([9999, 12, 31, 23, 59, 59]), 253402300799);
}

fn unixtime(t: [i128; 6]) -> i128 {
    let [mut y, m, d, h, mm, s] = t;
    if m <= 2 {
        y -= 1;
    }
