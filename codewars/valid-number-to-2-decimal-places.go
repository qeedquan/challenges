...
func main() {
	truthy := []string{"0.00", "3.90", "1000.00", ".00", "-2.55", "+2.10", "-.55"}
	falsy := []string{"hellow 11.99", "11.9", "11", "11.", ".9"}

	for _, input := range truthy {
		assert(DIGITS.MatchString(input) == true)
	}

	for _, input := range falsy {
		assert(DIGITS.MatchString(input) == false)
	}
}
...
var DIGITS = regexp.MustCompile(`^[+\-]?\d*\.\d\d$`)
