...
func vowelRecognition(_ input: String) -> Int {
    var count = 0
    var index = 0
    for symbol in input {
        switch symbol.lowercased() {
        case "a", "e", "i", "o", "u":
            count += (index + 1) * (input.count - index)
        default:
            break
        }
        index += 1
...
assert(vowelRecognition("baceb") == 16)
assert(vowelRecognition("aeiou") == 35)
assert(vowelRecognition("aeiouAEIOU") == 220)
assert(vowelRecognition("bbbb") == 0)
