...
void load_animation(const char *name, vector<SDL_FRect> &animation)
{
	animation.clear();
	...
	while (fgets(line, sizeof(line), fp))
	{
		...
		if (sscanf(line, "%31s %f %f %f %f", str, &rect.x, &rect.y, &rect.w, &rect.h) == 5)
...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Animated Sprites", width, height, wflag, &window, &renderer))
