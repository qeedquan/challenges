...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Fractal Dynamics", width, height, wflag, &window, &renderer))
...
void App::reset()
{
	...
	{
		float t = (i * 2 * SDL_PI_F) / max_points;
		points[i] = {
	...
	{
		cairo_arc(cr, point.x, point.y, 8, 0, 2 * SDL_PI_F);
		cairo_fill(cr);
	}
	...
	if (!paused)
	{
		for (auto i = 0; i < 1000; i++)
		{
			...
			if (next.x != previous.x || next.y != previous.y)
			{
				...
				cairo_arc(cr, current.x, current.y, 1, 0, 2 * SDL_PI_F);
				cairo_fill(cr);
			}
			previous = next;
		}
