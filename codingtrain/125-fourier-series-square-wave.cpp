...
void App::init()
{
	...
	auto width = 1024;
	auto height = 768;
	if (!SDL_CreateWindowAndRenderer("Fourier Series of Square Wave", width, height, 0, &window, &renderer))
...
void App::draw_epicycles_and_waves()
{
	...
	for (auto i = 0; i < harmonics; i++)
	{
		...
		SDL_RenderLine(renderer, prevx + xoff, prevy + yoff, x + xoff, y + yoff);
	}
	...
	xoff += 200;
	SDL_RenderLine(renderer, x + xoff - 200, y + yoff, xoff, wave[0] + yoff);
	for (auto i = 0u; i < wave.size() - 1; i++)
		SDL_RenderLine(renderer, i + xoff, wave[i] + yoff, i + xoff, wave[i + 1] + yoff);
}
