...
SDL_FPoint epicycles(SDL_Renderer *renderer, float x, float y, vector<Phasor> &phasors, float t, float rotation)
{
	for (auto &phasor : phasors)
	{
		...
		SDL_RenderLine(renderer, oldx, oldy, x, y);
	}
...
void App::init()
{
	...
	auto width = 1024;
	auto height = 768;
	if (!SDL_CreateWindowAndRenderer("Fourier Transform Drawing", width, height, 0, &window, &renderer))
...
void App::reload()
{
	...
	path.clear();
	phasor_time = 0;
	phasor_tick = (2 * SDL_PI_F) / x_phasors.size();
	finished = false;
...
void App::update_and_draw()
{
	...
	auto vy = epicycles(renderer, 100, height / 2.0, y_phasors, phasor_time, SDL_PI_F / 2);
	...
	SDL_RenderLine(renderer, vx.x, 0, vx.x, height);
	SDL_RenderLine(renderer, 0, vy.y, width, vy.y);

	for (auto i = 0u; i < path.size(); i++)
	{
		auto j = (i + 1) % path.size();
		SDL_RenderLine(renderer, path[i].x, path[i].y, path[j].x, path[j].y);
	}
