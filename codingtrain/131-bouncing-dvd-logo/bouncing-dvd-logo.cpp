...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Bouncing DVD Logo", width, height, wflag, &window, &renderer))
...
void App::reset()
{
	pos = { (width - dvd_width) / 2.0f, (height - dvd_height) / 2.0f };
	vel = { 5.0f, 5.0f };
	col = random_color();
...
void App::draw()
{
	...
	auto dstrect = SDL_FRect{ pos.x, pos.y, dvd_width * 1.0f, dvd_height * 1.0f };
	SDL_SetTextureColorMod(dvd, col.r, col.g, col.b);
	SDL_RenderTexture(renderer, dvd, NULL, &dstrect);
