...
initsdl(void)
{
	...
	w = 800;
	h = 600;
	if (!SDL_CreateWindowAndRenderer("Cardioid", w, h, SDL_WINDOW_RESIZABLE, &window, &renderer))
...
vector(int index, int total)
{
	...
	angle = linear_remap(index % total, 0, total, 0, 2 * SDL_PI_F);
...
cardioid(void)
{
	...
	cairo_arc(cr, 0, 0, radius, 0, 2 * SDL_PI_F);
	cairo_stroke(cr);
