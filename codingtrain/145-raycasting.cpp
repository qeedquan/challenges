...
{
	SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
	SDL_RenderLine(renderer, pos->x, pos->y, pos->x + (dir.x * 10), pos->y + (dir.y * 10));
}
...
void Particle::look(SDL_Renderer *renderer, vector<Boundary> &walls)
{
	for (auto i = 0u; i < rays.size(); i++)
	{
		...
		if (closest.x != -1)
		{
			...
			SDL_RenderLine(renderer, pos.x, pos.y, closest.x, closest.y);
		}
	}
}
...
void App::init()
{
	...
	width = 1024;
	height = 768;
	if (!SDL_CreateWindowAndRenderer("Raycasting", width, height, SDL_WINDOW_RESIZABLE, &window, &renderer))
