...
initsdl(void)
{
	...
	w = 800;
	h = 600;
	if (!SDL_CreateWindowAndRenderer("Supershape", w, h, SDL_WINDOW_RESIZABLE, &window, &renderer))
...
draw_supershape(void)
{
	...
	for (angle = 0; angle < 2 * SDL_PI_F; angle += increment) {
