...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Blobby", width, height, wflag, &window, &renderer))
...
void App::draw_blob()
{
	...
	auto radius = 0.4 * std::min(width, height);
	auto xoff = 0.0;
	for (auto a = 0.0; a <= 2 * SDL_PI_F; a += 0.1)
	{
		auto offset = perlin(vec2(xoff, yoff));
		offset = linear_remap(offset, -1, 1, -25, 25);
		auto r = radius + offset;
		auto x = r * cos(a);
		auto y = r * sin(a);
