...
{
	Cell c = {};
	c.position = { (float)(SDL_randf() * width), (float)(SDL_randf() * height) };
	c.radius = 60;
	c.color[0] = SDL_randf();
	c.color[2] = SDL_randf();
	c.color[3] = 0.39;
...
void Cell::move()
{
	position.x += linear_remap(SDL_randf(), 0, 1, -1, 1);
	position.y += linear_remap(SDL_randf(), 0, 1, -1, 1);
}

void Cell::draw(cairo_t *cr)
{
	cairo_set_source_rgba(cr, color[0], color[1], color[2], color[3]);
	cairo_arc(cr, position.x, position.y, radius, 0, 2 * SDL_PI_F);
	cairo_fill(cr);
}
...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Mitosis", width, height, wflag, &window, &renderer))
