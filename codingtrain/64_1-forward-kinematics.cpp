...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Forward Kinematics", width, height, wflag, &window, &renderer))
...
void App::update_and_draw()
{
	...
	for (auto segment = tentacle; segment; segment = segment->child)
	{
		...
		SDL_RenderLine(renderer, segment->a.x, segment->a.y, segment->b.x, segment->b.y);
	}
