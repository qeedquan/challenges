...
initsdl(void)
{
	...
	w = 800;
	h = 600;
	if (!SDL_CreateWindowAndRenderer("Clock", w, h, SDL_WINDOW_RESIZABLE, &window, &renderer))
...
draw_clock(void)
{
	...
	sa = linear_remap(tm.tm_sec, 0, 60, 0, 2 * SDL_PI_F);
	ma = linear_remap(tm.tm_min, 0, 60, 0, 2 * SDL_PI_F);
	ha = linear_remap(tm.tm_hour % 12, 0, 12, 0, 2 * SDL_PI_F);

	cairo_identity_matrix(cr);
	cairo_translate(cr, width / 2.0, height / 2.0);
	cairo_rotate(cr, -SDL_PI_F / 2);
	...
	cairo_arc(cr, 0, 0, 100, 0, 2 * SDL_PI_F);
	cairo_stroke(cr);
}
