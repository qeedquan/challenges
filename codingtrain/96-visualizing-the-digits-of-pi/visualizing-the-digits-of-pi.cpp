...
void circle(SDL_Renderer *renderer, float centerx, float centery, float radius, SDL_Color col)
{
	...
	auto dt = (2 * SDL_PI_F) / iterations;

	SDL_SetRenderDrawColor(renderer, col.r, col.g, col.b, col.a);
	for (auto t = 0.0f; t < 2 * SDL_PI_F; t += dt)
	{
...
void App::init()
{
	...
	if (!SDL_CreateWindowAndRenderer("Visualizing Digits of PI", width, height, wflag, &window, &renderer))
...
void App::next_line()
{
	...
	auto diff = (2 * SDL_PI_F) / 10;

	auto a1 = linear_remap(d1, 0, 10, 0, 2 * SDL_PI_F) + randn(-diff, diff);
	auto a2 = linear_remap(d2, 0, 10, 0, 2 * SDL_PI_F) + randn(-diff, diff);
