...
func main() {
	room_log := generate_foot_traffic(9, 4, 16, 300, 550)
	for i := range room_log {
		fmt.Println(room_log[i])
	}
}

// ported from @DEiE solution
func generate_foot_traffic(number_of_events, number_of_visitors, number_of_rooms, start_time, end_time int) []string {
	bench := make([]int, number_of_visitors)
	for i := range bench {
		bench[i] = i
	}

	events := []int{start_time}
	for range number_of_events*2 - 2 {
		events = append(events, rand_range(start_time+1, end_time-1))
	}
	events = append(events, end_time)
	sort.Ints(events[1 : len(events)-1])

	people_visiting_rooms := [][2]int{}
	room_log := []string{}

	for i := range number_of_events * 2 {
		move_someone_from_bench_to_room := func() {
			index := rand.IntN(len(bench))
			person := bench[index]
			room := rand.IntN(number_of_rooms)
			bench = slices.Delete(bench, index, index+1)
			people_visiting_rooms = append(people_visiting_rooms, [2]int{person, room})
			room_log = append(room_log, fmt.Sprintf("%v %v I %v", person, room, events[i]))
		}

		move_someone_from_room_to_bench := func() {
			index := rand.IntN(len(people_visiting_rooms))
			person := people_visiting_rooms[index][0]
			room := people_visiting_rooms[index][1]
			people_visiting_rooms = slices.Delete(people_visiting_rooms, index, index+1)
			bench = append(bench, person)
			room_log = append(room_log, fmt.Sprintf("%v %v O %v", person, room, events[i]))
		}
		...
		if number_of_events*2-i <= len(people_visiting_rooms) {
			...
			move_someone_from_room_to_bench()
		} else if len(bench) == number_of_visitors {
