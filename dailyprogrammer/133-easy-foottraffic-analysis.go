...
func analyze(logs [][4]int) error {
	visitors := make(map[int]int)
	starttime := make(map[int]int)
	totaltime := make(map[int]int)
	count := make(map[int]int)
	maxroom := 0
	for _, log := range logs {
		id, room, time := log[0], log[1], log[3]
		switch log[2] {
		case 'I':
			_, occupied := visitors[id]
			if occupied {
				return fmt.Errorf("invalid event %d: visitor %d is currently in a room: %d", log, id)
			}
			visitors[id] = room
			starttime[id] = time
		...
		case 'O':
			_, occupied := visitors[id]
			if !occupied && visitors[id] != room {
				return fmt.Errorf("invalid event %d: visitor %d is not in the room", log, id)
			}
			if starttime[id] > time {
				return fmt.Errorf("invalid event: %d enter time happens after exit time", log)
			}
			delete(visitors, id)
			totaltime[room] += time - starttime[id]
			count[room] += 1

		default:
			return fmt.Errorf("invalid event: %d", log)
		}
		maxroom = max(maxroom, room)
	}
	if len(visitors) != 0 {
		return fmt.Errorf("invalid log: there are visitor(s) still in room(s)")
	}

	for room := range maxroom + 1 {
		if count[room] != 0 {
			averagetime := totaltime[room] / count[room]
			fmt.Printf("Room %d, %d minute average visit, %d visitor(s) total\n", room, averagetime, count[room])
		}
	}
	fmt.Println()
