...
func simulate(nuclei []Nuclei, ticks, width, height int) {
	particles := make([][]int, height)
	for i := range particles {
		particles[i] = make([]int, width)
	}

	stream := bufio.NewWriter(os.Stdout)
	fmt.Fprintf(stream, "\x1b[2J")
	for tick := range ticks {
		for i := range nuclei {
			nuclei[i].count = 0
		}

		fmt.Fprintf(stream, "\x1b[H% 5d / % 5d\n", tick+1, ticks)
		for y := range height {
			for x := range width {
				index := &particles[y][x]
				nucleus := &nuclei[*index]
				if rand.Float64() < nucleus.lambda {
					*index += 1
				}
				fmt.Fprintf(stream, "\x1b[0;%dm%c\x1b[m", 31+*index, nucleus.code)
				nucleus.count += 1
			}
			fmt.Fprintln(stream)
		}
		...
		for i, nucleus := range nuclei {
			percentage := float64(nucleus.count) / float64(width*height)
			fmt.Fprintf(stream, "\x1b[0;%dm%c\x1b[m: %-10d %.2f%% \n",
				31+i, nucleus.code, nucleus.count, 100*percentage)
		}
		stream.Flush()
	}
}
