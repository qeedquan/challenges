...
func main() {
	fmt.Println(fight("hat", "cat"))
	fmt.Println(fight("miss", "hiss"))
	fmt.Println(fight("because", "cause"))
	fmt.Println(fight("hat", "cat"))
	fmt.Println(fight("hello", "below"))
	fmt.Println(fight("hit", "miss"))
	fmt.Println(fight("rekt", "pwn"))
	fmt.Println(fight("combo", "jumbo"))
	fmt.Println(fight("critical", "optical"))
	fmt.Println(fight("isoenzyme", "apoenzyme"))
	fmt.Println(fight("tribesman", "brainstem"))
	fmt.Println(fight("blames", "nimble"))
	fmt.Println(fight("yakuza", "wizard"))
	fmt.Println(fight("longbow", "blowup"))
}

func fight(s, t string) string {
	for _, c := range s + t {
		i := strings.IndexRune(s, c)
		j := strings.IndexRune(t, c)
		if i >= 0 && j >= 0 {
