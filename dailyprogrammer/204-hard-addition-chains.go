...
func main() {
	fmt.Println(solve(7, 43))
	fmt.Println(solve(9, 95))
	fmt.Println(solve(10, 127))
	fmt.Println(solve(13, 743))
	fmt.Println(solve(19, 123456))
	fmt.Println(solve(25, 1234567))
}

func solve(length, final int) []int {
	return recurse(length, final, []int{1})
}

func recurse(length, final int, nums []int) []int {
	for i1 := len(nums) - 1; i1 >= 0; i1-- {
		n1 := nums[i1]
		if n1*pow2(length+1-len(nums)) < final {
			break
		}

		for _, n2 := range nums[:i1+1] {
			sum := n1 + n2
			...
			if len(nums) == length {
				if sum == final {
					return append(nums, sum)
				} else {
					continue
				}
			}

			result := recurse(length, final, append(nums, sum))
			if len(result) > 0 {
				return result
			}
		}
