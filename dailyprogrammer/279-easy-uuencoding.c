...
uudecode()
{
	...
	while (fgets(line, sizeof(line), stdin) && line[0] != '`') {
		byte_count = line[0] - 32;
		char_count = byte_count * 4 / 3;
		for (i = 0; i < char_count; i += 4) {
			bytes[0] = ((line[i + 1] - 32) << 2) | ((line[i + 2] - 32) >> 4);
			bytes[1] = (((line[i + 2] - 32) & 0xf) << 4) | ((line[i + 3] - 32) >> 2);
			bytes[2] = (((line[i + 3] - 32) & 0x3) << 6) | ((line[i + 4] - 32));

			out_count = 3;
			if (i + 4 > char_count)
				out_count = byte_count % 3;
