...
func solve(dx, dy, xmag, gx, gy, vmag float64, gp [][2]float64) [2]float64 {
	...
	for _, p := range gp {
		...
		deltax := gx - x
		deltay := gy - y
		mag2 := deltax*deltax + deltay*deltay
		if mag2 < best {
			ex, ey, best = x, y, mag2
		}
	}
	...
	vx := ex - gx
	vy := ey - gy
	tmp := math.Hypot(vx, vy)
	vx = vmag * vx / tmp
	vy = vmag * vy / tmp

	h1 := vx*vx + vy*vy - xmag*xmag
	h2 := ox*vx + oy*vy
	h3 := ox*ox + oy*oy
	p2 := math.Sqrt((h2/h1)*(h2/h1) - h3/h1)
	p1 := h2 / h1
	t1 := p1 + p2
	t2 := p1 - p2
	tmax := math.Sqrt((ex-gx)*(ex-gx)+(ey-gy)*(ey-gy)) / vmag
	if t1 >= 0 && t1 <= tmax {
		return [2]float64{gx + vx*t1, gy + vy*t1}
	} else if t2 >= 0 && t2 <= tmax {
		return [2]float64{gx + vx*t2, gy + vy*t2}
	}
