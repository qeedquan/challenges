...
function subsetsum(array) {
	return array.some(x => array.some(y => x + y === 0));
}

function main() {
	assert(subsetsum([1, 2, 3]) == false);
	assert(subsetsum([-5, -3, -1, 2, 4, 6]) == false);
	assert(subsetsum([]) == false);
	assert(subsetsum([-1, 1]) == true);
	assert(subsetsum([-97364, -71561, -69336, 19675, 71561, 97863]) == true);
	assert(subsetsum([-53974, -39140, -36561, -23935, -15680, 0]) == true);
}
