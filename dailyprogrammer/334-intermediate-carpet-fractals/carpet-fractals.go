...
func main() {
	...
	rules, ncolors, size, err := decode(flag.Arg(0))
	check(err)

	fd, err := os.Create(flag.Arg(1))
	check(err)
	defer fd.Close()

	stream := bufio.NewWriter(fd)
	defer stream.Flush()
	render(stream, rules, ncolors, size, flags.scale)
}
...
func parseflags() {
	flag.IntVar(&flags.scale, "scale", 9, "scale factor")
	flag.Usage = usage
	flag.Parse()
	if flag.NArg() != 2 {
		usage()
...
func usage() {
	fmt.Fprintln(os.Stderr, "usage: art.txt out.pgm")
	flag.PrintDefaults()
	os.Exit(2)
}

func decode(name string) ([][9]int, int, int, error) {
	fd, err := os.Open(name)
	if err != nil {
		return nil, 0, 0, err
	}
	...
	var ncolors, niters int
	fmt.Fscanf(fd, "%d %d", &ncolors, &niters)

	var rules [][9]int
	for range ncolors {
		var rule [9]int
		for i := range rule {
			fmt.Fscanf(fd, "%d", &rule[i])
		}
	...
	return rules, ncolors, size, nil
}

func render(stream io.Writer, rules [][9]int, ncolors, size, scale int) {
	fmt.Fprintf(stream, "P2\n%d %d\n%d\n", size*scale, size*scale, ncolors-1)
	for y := range size * scale {
		for x := range size * scale {
			px := x / scale
			py := y / scale
			shade := 0
			for div := size / 3; div > 0; div /= 3 {
				shade = rules[shade][py/div*3+px/div]
				px %= div
				py %= div
			}
			fmt.Fprintln(stream, shade)
		}
	}
}
