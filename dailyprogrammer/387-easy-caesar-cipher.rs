...
fn main() {
    assert_eq!(caesar("a", 1), "b");
    assert_eq!(caesar("abcz", 1), "bcda");
    assert_eq!(caesar("irk", 13), "vex");
    assert_eq!(caesar("fusion", 6), "layout");
    assert_eq!(caesar("dailyprogrammer", 6), "jgorevxumxgsskx");
    assert_eq!(caesar("jgorevxumxgsskx", 20), "dailyprogrammer");
    assert_eq!(caesar("Daily Programmer!", 6), "Jgore Vxumxgsskx!");
}

fn caesar(input: &'static str, shift: u32) -> String {
    let mut output = String::new();
    for mut symbol in input.chars() {
        if symbol.is_ascii_alphabetic() {
            ...
            let encoded = ((symbol as u32 - start + shift) % 26) + start;
            let encoded = char::from_u32(encoded);
            symbol = encoded.unwrap();
        }
        output.push(symbol);
    }
    output
