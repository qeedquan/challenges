...
func main() {
	test(19, []uint{19, 25, 28, 14, 7})
	test(69, []uint{69, 98, 49, 56, 28, 14, 7})
	test(205, []uint{205, 230, 115, 121, 124, 62, 31})
	test(357, []uint{357, 434, 217, 236, 118, 59, 61, 62, 31})
	test(54321, []uint{54321, 59928, 29964, 14982, 7491, 7841, 8016, 4008, 2004, 1002, 501, 506, 253, 254, 127})
}
...
func test(x uint, r []uint) {
	p := brs(x)
	fmt.Println(p)
	assert(reflect.DeepEqual(p, r))
}

func brs(x uint) []uint {
	r := []uint{}
	for {
		r = append(r, x)
		y := rot(x)
...
func rot(x uint) uint {
	if x == 0 {
		return 0
	}

	b := bits.UintSize - bits.LeadingZeros(x) - 1
	return (x&1)<<b | x>>1
}
