...
bool error_callback(const TErrorInfo &)
{
	return false;
}
...
int main()
{
	Lexer lexer(make_unique<StringInputStream>(input));
	Preprocessor preprocessor(lexer, { error_callback });
	cout << Preprocessor::ToString(preprocessor.Process()) << endl;
	return 0;
}
