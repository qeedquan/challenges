...
func main() {
	assert(score([]string{"jump", "add", "add", "jump", "add", "jump"}, []int{2, 1, 3, 1, -2, -3}) == 1)
	assert(score([]string{"jump", "add", "add"}, []int{3, 1, 1}) == 0)
	assert(score([]string{"jump"}, []int{0}) == 0)
}
...
func score(instructions []string, values []int) int {
	result := 0
	seen := make(map[int]bool)
	ip := 0
	for 0 <= ip && ip < len(instructions) {
		if seen[ip] {
			break
		}
		seen[ip] = true
