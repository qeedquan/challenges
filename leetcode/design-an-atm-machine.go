...
func main() {
	atm := NewATM()
	atm.Deposit([5]int{0, 0, 1, 2, 1})
	assert(slices.Equal(atm.Withdraw(600), []int{0, 0, 1, 0, 1}))
	atm.Deposit([5]int{0, 1, 0, 1, 1})
	assert(slices.Equal(atm.Withdraw(600), []int{-1}))
	assert(slices.Equal(atm.Withdraw(550), []int{0, 1, 0, 0, 1}))
}
...
func (c *ATM) Deposit(notes [5]int) {
	for i := range notes {
		c.count[i] += notes[i]
	}
}
...
func (c *ATM) Withdraw(amount int) []int {
	result := make([]int, len(c.count))
	for i := len(c.values) - 1; i >= 0; i-- {
		result[i] = min(amount/c.values[i], c.count[i])
		amount -= result[i] * c.values[i]
	}
