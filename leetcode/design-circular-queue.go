...
func main() {
	c := NewCQ(3)
	assert(c.Enqueue(1) == true)
	assert(c.Enqueue(2) == true)
	assert(c.Enqueue(3) == true)
	assert(c.Enqueue(4) == false)
	assert(c.Rear() == 3)
	assert(c.IsFull() == true)
	assert(c.Dequeue() == true)
	assert(c.Enqueue(4) == true)
	assert(c.Rear() == 4)
}
...
type CQ[T int] struct {
	start int
	size  int
	data  []T
}

func NewCQ[T int](size int) *CQ[T] {
	return &CQ[T]{
		data: make([]T, size),
	}
}

func (c *CQ[T]) Enqueue(value T) bool {
	if c.IsFull() {
		return false
	}
...
func (c *CQ[T]) IsEmpty() bool {
	return c.size == 0
}

func (c *CQ[T]) IsFull() bool {
