...
fn main() {
    assert_eq!(bad(&vec![2, 3, 4, 5], &vec![2, 1, 3, 4]), 1);
    assert_eq!(bad(&vec![2, 2, 2, 2, 2], &vec![2, 2, 2, 2, 5]), -1);
    assert_eq!(bad(&vec![2, 3, 2, 2, 3, 2], &vec![2, 3, 2, 3, 2, 7]), 2);
}

fn bad(a: &Vec<isize>, b: &Vec<isize>) -> isize {
    let r0 = replaceable(b, a);
    let r1 = replaceable(a, b);
    match (r0, r1) {
        (true, true) => -1,
        (false, false) => -1,
        (true, false) => 1,
...
fn replaceable(a: &Vec<isize>, b: &Vec<isize>) -> bool {
    let (mut i, mut j, mut d) = (0, 0, -1);
    let (n, m) = (a.len(), b.len());
    while i < n && j < m {
        if a[i] == b[j] {
            i += 1;
