...
The veganFriendly filter will be either true (meaning you should only include restaurants with veganFriendly[i] set to true) or false (meaning you can include any restaurant).
...
func main() {
	test(
		[][5]int{{1, 4, 1, 40, 10}, {2, 8, 0, 50, 5}, {3, 8, 1, 30, 4}, {4, 10, 0, 10, 3}, {5, 1, 1, 15, 1}}, 1, 50, 10,
		[]int{3, 1, 5},
	)

	test(
		[][5]int{{1, 4, 1, 40, 10}, {2, 8, 0, 50, 5}, {3, 8, 1, 30, 4}, {4, 10, 0, 10, 3}, {5, 1, 1, 15, 1}}, 0, 50, 10,
		[]int{4, 3, 2, 1, 5},
	)

	test(
		[][5]int{{1, 4, 1, 40, 10}, {2, 8, 0, 50, 5}, {3, 8, 1, 30, 4}, {4, 10, 0, 10, 3}, {5, 1, 1, 15, 1}}, 0, 30, 3,
		[]int{4, 5},
	)
}
...
func test(restaurants [][5]int, vegan, maxprice, maxdistance int, expected []int) {
	ids := filter(restaurants, vegan, maxprice, maxdistance)
	fmt.Println(ids)
	assert(reflect.DeepEqual(ids, expected))
}

func filter(restaurants [][5]int, vegan, maxprice, maxdistance int) []int {
	var records [][5]int
	for _, restaurant := range restaurants {
		if restaurant[2] == 0 && vegan != 0 {
			continue
		}
