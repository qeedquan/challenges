...
func test(graph [][]int, expected []int) {
	result := safenodes(graph)
	fmt.Println(result)
}

func safenodes(graph [][]int) []int {
	lookup := make([]int, len(graph))
	result := []int{}
	for node := range graph {
		if dfs(graph, lookup, node) {
...
func dfs(graph [][]int, lookup []int, node int) bool {
