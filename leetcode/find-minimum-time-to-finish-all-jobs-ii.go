...
You are given two 0-indexed integer arrays jobs and workers of equal length, where jobs[i] is the amount of time needed to complete the ith job,
...
func main() {
	assert(time([]int{5, 2, 4}, []int{1, 7, 5}) == 2)
	assert(time([]int{3, 18, 15, 9}, []int{6, 5, 1, 3}) == 3)
}
...
func time(jobs, workers []int) int {
	sort.Ints(jobs)
	sort.Ints(workers)

	result := 0
	for i := range jobs {
		result = max(result, ceildiv(jobs[i], workers[i]))
	}
	return result
}
