...
Hint 2
Let x be the current mana value. Starting from now = f[0], update now = max(now + skill[i - 1] * x, f[i]) for i in [1..n].
Then, the final f[n - 1] = now + skill[n - 1] * x for this potion.
...
func main() {
	assert(minimize([]int{1, 5, 2, 4}, []int{5, 1, 4, 2}) == 110)
	assert(minimize([]int{1, 1, 1}, []int{1, 1, 1}) == 5)
	assert(minimize([]int{1, 2, 3, 4}, []int{1, 2}) == 21)
}
...
func minimize(skill, mana []int) int {
	if len(skill) == 0 || len(mana) == 0 {
		return 0
	}

	result := 0
	for i := 1; i < len(mana); i++ {
		prefix := 0
		maximum := 0
		for j := range skill {
			prefix += skill[j]
			maximum = max(maximum, mana[i-1]*prefix-mana[i]*(prefix-skill[j]))
		}
		result += maximum
	}
	...
	result += mana[len(mana)-1] * multiplier
