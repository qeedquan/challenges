...
fn main() {
    assert_eq!(chalk_replace(&vec![5, 1, 5], 22), 0);
    assert_eq!(chalk_replace(&vec![3, 4, 1, 2], 25), 1);
}

fn chalk_replace(chalk: &Vec<isize>, mut k: isize) -> isize {
    k %= chalk.iter().sum::<isize>();
    if k == 0 {
        return 0;
    }
