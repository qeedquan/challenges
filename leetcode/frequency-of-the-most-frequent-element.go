...
func main() {
	assert(maxfrequency([]int{1, 2, 4}, 5) == 3)
	assert(maxfrequency([]int{1, 4, 8, 13}, 5) == 2)
	assert(maxfrequency([]int{3, 9, 6}, 2) == 1)
}
...
func maxfrequency(numbers []int, k int) int {
	sort.Ints(numbers)
	left := 0
	right := 0
	total := k
	for ; right < len(numbers); right++ {
		total += numbers[right]
		if total < numbers[right]*(right-left+1) {
			total -= numbers[left]
			left += 1
		}
	}
