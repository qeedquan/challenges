...
func main() {
	assert(bags([]int{2, 3, 4, 5}, []int{1, 2, 4, 4}, 2) == 3)
	assert(bags([]int{10, 2, 2}, []int{2, 2, 0}, 100) == 3)
}
...
func bags(capacity, rocks []int, additional int) int {
	for i := range len(capacity) {
		capacity[i] -= rocks[i]
	}
	sort.Ints(capacity)
	for i := range capacity {
		if capacity[i] > additional {
			return i
		}
		additional -= capacity[i]
	}
	return len(capacity)
}
