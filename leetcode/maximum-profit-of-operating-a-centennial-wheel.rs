...
fn main() {
    assert_eq!(max_profit(&[8, 3], 5, 6), 3);
    assert_eq!(max_profit(&[10, 9, 6], 6, 4), 7);
    assert_eq!(max_profit(&[3, 4, 0, 5, 1], 1, 92), -1);
}

fn max_profit(customers: &[isize], boarding_cost: isize, running_cost: isize) -> isize {
    let mut max_run = -1;
    let mut max_prof = 0;
    let mut run = 1;
    let mut prof = 0;
    let mut waiting = 0;
    let mut index = 0;
    while index < customers.len() || waiting > 0 {
        ...
        waiting -= boarding;
        prof += (boarding * boarding_cost) - running_cost;
        if prof > max_prof {
            max_prof = prof;
            max_run = run;
        }
