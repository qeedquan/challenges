...
def merge_triplets(triplets, target):
    ...
    for triplet in triplets:
        if not (triplet[0] <= target[0] and triplet[1] <= target[1] and triplet[2] <= target[2]):
            continue
        
        for index in range(len(result)):
            result[index] = max(result[index], triplet[index])
...
def main():
    ...
    assert(merge_triplets([[2, 5, 3], [2, 3, 4], [1, 2, 5], [5, 2, 3]], [5, 5, 5]) == True)
