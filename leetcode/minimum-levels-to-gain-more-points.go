...
Hint 1
Change all 0 in possible array into -1.
...
func main() {
	assert(levels([]int{1, 0, 1, 0}) == 1)
	assert(levels([]int{1, 1, 1, 1, 1}) == 3)
	assert(levels([]int{0, 0}) == -1)
}
...
func levels(possible []int) int {
	prefix := make([]int, len(possible)+1)
	...
	for i := range len(possible) - 1 {
		if prefix[i+1] > prefix[len(prefix)-1]-prefix[i+1] {
			return i + 1
		}
	}
	return -1
