...
func main() {
	assert(frequent([]int{1, 100, 200, 1, 100}, 1) == 100)
	assert(frequent([]int{2, 2, 2, 2, 3}, 2) == 2)
}
...
func frequent(numbers []int, key int) int {
	count := make(map[int]int)
	result := 0
	for index := 0; index+1 < len(numbers); index++ {
		if numbers[index] == key {
			current := numbers[index+1]
			count[current] += 1
			if count[result] < count[current] {
				result = current
			}
		}
	}
