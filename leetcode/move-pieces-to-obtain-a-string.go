...
func main() {
	assert(change("_L__R__R_", "L______RR") == true)
	assert(change("R_L_", "__LR") == false)
	assert(change("_R", "R_") == false)
}
...
func change(s, t string) bool {
	...
	for {
		...
		if i == len(s) || j == len(t) || s[i] != t[j] || (s[i] == 'L' && i < j) || (s[i] == 'R' && i > j) {
			return false
		}
		i += 1
		j += 1
