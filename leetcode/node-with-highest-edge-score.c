...
ssize_t
edgescore(int *edges, ssize_t nedges)
{
	ssize_t edgeidx;
	ssize_t i;
	int *score;
	...
	score = calloc(nedges, sizeof(*score));
	if (!score)
		return -errno;
	...
	for (i = 0; i < nedges; i++) {
		score[edges[i]] += i;
		if (score[edgeidx] < score[edges[i]] || (score[edgeidx] == score[edges[i]] && edges[i] < edgeidx))
			edgeidx = edges[i];
	}
...
main()
{
	...
	assert(edgescore(edges1, nelem(edges1)) == 7);
	assert(edgescore(edges2, nelem(edges2)) == 0);
