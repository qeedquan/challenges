...
Hint 2
If possible, we could sell the m x n piece. We could also cut the piece vertically creating two pieces of size m x n1 and m x n2 where n1 + n2 = n,
or horizontally creating two pieces of size m1 x n and m2 x n where m1 + m2 = m.

Hint 3
Notice that cutting a piece breaks the problem down into smaller subproblems, and selling the piece when available is also a case that terminates the process.
Thus, we can use DP to efficiently solve this.
...
func main() {
	assert(sellingwood(3, 5, [][3]int{{1, 4, 2}, {2, 2, 7}, {2, 1, 3}}) == 19)
	assert(sellingwood(4, 6, [][3]int{{3, 2, 10}, {1, 4, 2}, {4, 1, 3}}) == 32)
}
...
func sellingwood(m, n int, p [][3]int) int {
	d := make([][]int, m+1)
	for i := range d {
		d[i] = make([]int, n+1)
	}
