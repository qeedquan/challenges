...
func main() {
	assert(calculate("abcdefghijklmnopqrstuvwxyz", "cba") == 4)
	assert(calculate("pqrstuvwxyzabcdefghijklmno", "leetcode") == 73)
}
...
func calculate(keyboard, word string) int {
	lookup := make(map[rune]int)
	for index, key := range keyboard {
		lookup[key] = index
	}
	...
	for _, symbol := range word {
		result += abs(lookup[symbol] - previous)
		previous = lookup[symbol]
	}
	return result
}
