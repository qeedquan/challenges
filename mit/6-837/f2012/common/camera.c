...
camera_matrix(Camera *c, float4x4 m)
{
	...
	lookat4x4(l, (float3){{0, 0, c->distance}}, (float3){{0, 0, 0}}, (float3){{0, 1, 0}});
...
arcball_rotate_camera(Camera *c, float2 p, float s)
{
	...
	d = sx * ex + sy * ey + sz * ez;
	if (d != 1) {
		axis = (float3){{sy * ez - ey * sz, sz * ex - ex * sx, sx * ey - ey * sy}};
...
recalc_projection_matrix(GC *gc)
{
	...
	cam = &gc->camera;
	cam->viewport = (float4){{0, 0, gc->width, gc->height}};
