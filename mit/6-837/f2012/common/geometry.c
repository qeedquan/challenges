...
gen_circle_curve(Curve *c, float r, size_t n)
{
	...
	for (i = 0; i <= n; i++) {
		t = 2.0 * M_PI * i / n;
		c->vertices[i] = (float3){{r * cos(t), r * sin(t), 0}};
		c->tangents[i] = (float3){{-sin(t), cos(t), 0}};
		c->normals[i] = (float3){{-cos(t), -sin(t), 0}};
		c->binormals[i] = (float3){{0, 0, 1}};
	}
}
