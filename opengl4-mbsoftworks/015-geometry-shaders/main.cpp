...
public:
	...
	void render(AmbientLight &ambient_light, DiffuseLight &diffuse_light, bool display_normals, float normal_length);

	FlyingCamera *camera;
};

void HUD015::init(GLWindow *glwindow, FlyingCamera *camera)
{
	HUD::init(glwindow);
	this->camera = camera;
...
void HUD015::render(AmbientLight &ambient_light, DiffuseLight &diffuse_light, bool display_normals, float normal_length)
{
	auto &fm = FontManager::get_instance();
	...
	y += font.pixel_size;
	font.print(x, y, "Display normals: %s (Press 'N' to toggle)",
			   (display_normals) ? "Yes" : "No");
	y += font.pixel_size;
	font.print(x, y, "Normals length: %.3f (Press '+' or '-' to change length)", normal_length);
}
...
void App::render()
{
	...
	vector<Mat4f> pyramid_model_matrices;
	for (auto &position : pyramid_positions)
	{
		...
		pyramid_model_matrices.push_back(model);
		program.set_model_and_normal_matrix(model);
	...
	if (display_normals)
	{
		...
		normals_program["normalLength"] = normal_length;
		...
		matrix_it = pyramid_model_matrices.cbegin();
		for (size_t i = 0; i < pyramid_positions.size(); i++)
		{
			normals_program.set_model_and_normal_matrix(*matrix_it++);
			pyramid.render_points();
		}
	...
	// Render HUD
	hud->render(ambient_light, diffuse_light, display_normals, normal_length);
...
void App::event(SDL_Event *ev)
{
	...
	camera.window_center_position = Vec2f{ w / 2.0f, h / 2.0f };

	auto key_input = [&](SDL_Keycode key) {
		return ev->type == SDL_EVENT_KEY_DOWN && ev->key.key == key;
	};
	auto get_cursor_pos = [&]() {
		float x, y;
...
int main()
{
	auto title = "015.) Geometry Shaders - Tutorial by Michal Bubnar (www.mbsoftworks.sk)";
	auto major = 4;
	auto minor = 4;
