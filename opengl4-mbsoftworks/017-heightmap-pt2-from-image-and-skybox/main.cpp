...
void HUD017::init(GLWindow *glwindow, FlyingCamera *camera)
{
	HUD::init(glwindow);
	this->camera = camera;
...
void HUD017::render(bool display_normals)
{
	auto &fm = FontManager::get_instance();
...
void App::load()
{
	...
	tm.add_texture("sand", "data/textures/sand.png");
	...
	heightmap->init("data/heightmaps/tut017.png", true, true, true);
...
int main()
{
	auto title = "017.) Heightmap Pt.2 - From Image and Skybox - Tutorial by Michal Bubnar (www.mbsoftworks.sk)";
	auto major = 4;
	auto minor = 4;
