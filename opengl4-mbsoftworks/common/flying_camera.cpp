...
void FlyingCamera::update(const function<bool(SDL_Keycode)> &key_input,
						  const function<Vec2f()> &get_cursor_pos,
						  const function<void(Vec2f &)> &set_cursor_pos,
						  const function<float(float)> &speed_correction)
{
	...
	if (delta.x != 0)
		rotate_left_right(delta.x * mouse_sensitivity);

	if (delta.y != 0)
		rotate_up_down(delta.y * mouse_sensitivity);
