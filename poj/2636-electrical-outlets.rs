...
fn main() {
    assert_eq!(outlets(&vec![2, 3, 4]), 7);
    assert_eq!(outlets(&vec![4, 4, 4, 4, 4, 4, 4, 4, 4, 4]), 31);
    assert_eq!(outlets(&vec![10, 10, 10, 10]), 37);
}

fn outlets(a: &Vec<usize>) -> usize {
    let r: usize = a.iter().sum();
    let n = a.len();
    if r >= n {
        r - n + 1
    } else {
