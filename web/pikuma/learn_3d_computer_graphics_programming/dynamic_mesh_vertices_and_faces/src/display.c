...
initialize_window(void)
{
	...
	if (!dm) {
		fprintf(stderr, "Failed to get current display: %s\n", SDL_GetError());
		return false;
	}
...
void
draw_line(int x0, int y0, int x1, int y1, uint32_t color)
{
	int delta_x = (x1 - x0);
	int delta_y = (y1 - y0);
...
void
draw_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
